...
├─────────────────────────────────────────────────────────────┤
│  MainArea（横並び）                                          │
│  ┌─ Timeline（縦スクロール可能） ──────┐┌─ Statistics ────┐ │
│  │  方面ラベル │ 時間軸 │ A枠 │ B枠    ││  方面別統計テーブル │ │
│  │  （縦列）   │（縦軸）│レーン│レーン   ││                  │ │
│  │            │       │（縦）│（縦）   ││                  │ │
│  │  上=100s ──────────────────→       ││                  │ │
│  │  下=0s   ──────────────────→       ││                  │ │
...
- Timeline: 残り幅を占有（flex-1）。内部コンテンツは縦スクロール
...
| タイムライン全高 | 1600px | 100秒 × 16px |
| 方面ラベル列幅 | 32px | テキスト表示 + 編集に十分 |
| 時間軸列幅 | 28px | 目盛り + ラベル |
| 枠レーン幅 | 80px | マーカー + 活動期間バー + 余白 |
| マーカー直径 | 20px | クリック・ドラッグ操作に十分な大きさ |
| 活動期間バー幅 | 8px | マーカーの横に配置 |
| レーン間スペース | 4px | 視覚的分離 |

タイムライン全体の幅: 32 + 28 + 80 + 4 + 80 = 224px（B枠あり）

### 3.2 スクロール

タイムラインコンテナは `overflow-y: auto` で縦スクロール可能。画面の高さに収まらない 1600px のタイムラインを操作可能にする。

水平方向のスクロールは不要（タイムライン幅は固定で画面内に収まる）。

### 3.3 時間軸（TimeAxis）

- 上端 = 100秒（ゲーム開始）、下端 = 0秒（ゲーム終了）
- 主目盛り: 10秒ごとにラベル付き横線
- 副目盛り: 5秒ごとに短い横線（ラベルなし）
...
| ラベル | 湧き時刻（秒）をマーカー上側に表示 |
| 方面表示 | マーカー下側に方面名を小さく表示 |
| 操作 | なし（読み取り専用） |
...
- タイムラインは縦方向に1600pxのコンテンツを持ち、縦スクロールで操作する
- タイムラインの幅は固定（約224px、B枠あり時）で画面内に収まる
- SettingsPanel の折りたたみにより、タイムラインの縦表示領域を最大化可能
